...
def generate_number():
    ...
    if empty_cells:
        ...
        canvas.itemconfig(单元格[row * 列 + col], fill=颜色映射["2"])  # 填充颜色
...
def move_up():
    for col in range(列):
        ...
        for row in range(行):
            ...
            update_cell_color(row, col, new_column[row])  # 更新颜色
    generate_number()
    check_game_over()
...
def move_left():
    for row in range(行):
        ...
        for col in range(列):
            ...
            update_cell_color(row, col, new_row[col])  # 更新颜色
    generate_number()
    check_game_over()
